...
terraform {
  source = "git::git@github.com:pagopa/io-infrastructure-modules-new.git//azurerm_private_endpoint?ref=update-azurerm-v2.87.0"
}

inputs = {
  name                = "${dependency.storage_account.outputs.resource_name}-queue-endpoint"
  resource_group_name = dependency.resource_group.outputs.resource_name
  subnet_id           = dependency.subnet_pendpoints.outputs.id

  private_service_connection = {
    name                           = "${dependency.storage_account.outputs.resource_name}-queue"
