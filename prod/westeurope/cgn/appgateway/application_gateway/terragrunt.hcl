...
dependency "function_cgn" {
  config_path = "../../functions_cgn/function_app"
}

# external
dependency "firewall_custom_rules" {
  config_path = "../../../external/firewall_custom_rules"
}
...
inputs = {
  ...
  backend_address_pools = [
    {
      ...
      fqdns = [
        dependency.function_cgn.outputs.default_hostname,
      ]
      ip_addresses = []
    },
  ]
  ...
  http_listeners = [{
    ...
    protocol                       = "Https"
    host_name                      = "cgn-test.io.italia.it"
    # Note the certificate name can not contain dot.
    ssl_certificate_name = "cgn-test-io-italia-it"
    require_sni          = true
  }]
